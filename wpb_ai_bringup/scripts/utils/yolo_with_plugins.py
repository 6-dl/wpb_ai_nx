...
plugins_path = home_path + '/plugins/libyolo_layer.so'
try:
    # ctypes.cdll.LoadLibrary('/home/robot/plugins/libyolo_layer.so')
    ctypes.cdll.LoadLibrary(plugins_path)
except OSError as e:
    raise SystemExit('ERROR: failed to load  ' + plugins_path) from e
...
class TrtYOLO(object):
    ...
    def _load_engine(self):
        home_path = os.path.expanduser('~')
        #TRTbin = '/home/robot/model/%s.trt' % self.model
        TRTbin = '/model/%s.trt' % self.model
        TRTbin = home_path + TRTbin
